...
function Api(
  ...
  const service: AxiosInstance = axios.create({
    baseURL: '/api', // 本地调试时候使用
    // baseURL: 'http://127.0.0.1:9098', // 设置统一的请求前缀
    // baseURL: '', // 设置统一的请求前缀
...
function getPendingKey(config: any) {
  ...
  console.log(666.789, params, data);
  if (typeof data === 'string') {
    data = JSON.parse(data);
  }
  return [url, method, toStr(params), toStr(data)].join('&');
}
