...
class TransformControls extends Object3D {
  constructor(camera, domElement) {
    ...
    const _gizmo = new TransformControlsGizmo(); //（Notic：小控件）
    this._gizmo = _gizmo;
    this.add(_gizmo);

    const _plane = new TransformControlsPlane(); //（Notic：小飞机？）
    this._plane = _plane;
    this.add(_plane);
  ...
  // updateMatrixWorld  updates key transformation variables 更新关键转换变量
  updateMatrixWorld() {
    ...
    if (mode === "translate") {
      ...
      if (axis === "E") {
        ...
        this.rotationAngle =
          ...
          ROTATION_SPEED;
        // console.log( this.rotationAngle, "测试是不是可以输出值", object.rotation);
      }
      ...
      if (space === "local" && axis !== "E" && axis !== "XYZE") {
        ...
        //本质上是利用轴角+四元数的方式处理旋转，它提供了预置常规旋转轴单位向量的rotateX、rotateY等方便调用的方法
...
function onPointerHover(event) {
  // console.log("onPointerHoveronPointerHover");
  if (!this.enabled) return;
...
class TransformControlsGizmo extends Object3D {
  constructor() {
    ...
    const gizmoTranslate = {
      X: [
        ...
        // [ new Mesh( arrowGeometry, matRed ), [ - 0.5, 0, 0 ], [ 0, 0, Math.PI / 2 ]], //1118-修改-去除负坐标的箭头
        [new Mesh(lineGeometry2, matRed), [0, 0, 0], [0, 0, -Math.PI / 2]],
      ],
      Y: [
        [new Mesh(arrowGeometry, matGreen), [0, 0.5, 0]],
        // [ new Mesh( arrowGeometry, matGreen ), [ 0, - 0.5, 0 ], [ Math.PI, 0, 0 ]], //1118-修改-去除负坐标的箭头
        [new Mesh(lineGeometry2, matGreen)],
      ],
      Z: [
        [new Mesh(arrowGeometry, matBlue), [0, 0, 0.5], [Math.PI / 2, 0, 0]],
        // [ new Mesh( arrowGeometry, matBlue ), [ 0, 0, - 0.5 ], [ - Math.PI / 2, 0, 0 ]], //1118-修改-去除负坐标的箭头
    ...
    const pickerTranslate = {
      X: [
        ...
        // [ new Mesh( new CylinderGeometry( 0.2, 0, 0.6, 4 ), matInvisible ), [ - 0.3, 0, 0 ], [ 0, 0, Math.PI / 2 ]] //1118-修改-去除负坐标的箭头
      ],
      Y: [
        [
          new Mesh(new CylinderGeometry(0.2, 0, 0.6, 4), matInvisible),
        ...
        // [ new Mesh( new CylinderGeometry( 0.2, 0, 0.6, 4 ), matInvisible ), [ 0, - 0.3, 0 ], [ 0, 0, Math.PI ]] //1118-修改-去除负坐标的箭头
      ],
      Z: [
        [
          new Mesh(new CylinderGeometry(0.2, 0, 0.6, 4), matInvisible),
        ...
        // [ new Mesh( new CylinderGeometry( 0.2, 0, 0.6, 4 ), matInvisible ), [ 0, 0, - 0.3 ], [ - Math.PI / 2, 0, 0 ]] //1118-修改-去除负坐标的箭头
      ],
    ...
    const gizmoRotate = {
      // XYZE: [
      // 	[ new Mesh( CircleGeometry( 0.5, 1 ), matGray ), null, [ 0, Math.PI / 2, 0 ]] //1122-修改-不在需要全部转动
      // ],
    ...
    const pickerRotate = {
      // XYZE: [
      // 	[ new Mesh( new SphereGeometry( 0.25, 10, 8 ), matInvisible ) ] //1122-修改-不在需要全部轴转动
      // ],
      X: [
        [
    ...
    // Creates an Object3D with gizmos described in custom hierarchy definition. 使用自定义层次定义中描述的小控件创建Object3D。
    // 创建可以操控的小控件
  ...
  // updateMatrixWorld will update transformations and appearance of individual handles 更新各个句柄的转换和外观
